...
router.post("/servicerequest", verify, async (req, res) => {
  ...
  try {
    ...
    if (error) return res.status(400).send(error.details[0].message);
      ...
      const users = await User.find({ type: { $in: ["admin", "manager"] }}).exec();

      users.forEach(user => {
           sendEmail(user.email, "CRM Notification Mail", `Service Request ${saveTicket.title} has been Created for Client ${saveTicket.client}`);
      });
...
router.post("/lead", verify, async (req, res) => {
  ...
  try {
    ...
    if (error) return res.status(400).send(error.details[0].message);
      ...
      users.forEach(user => {
           sendEmail(user.email, "CRM Notification Mail", `Lead ${leads.title} has been Created for Client ${leads.client}`);
      });
